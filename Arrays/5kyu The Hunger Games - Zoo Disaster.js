...
// Loop over each element in zoo and check if it eats anything around it.
    // If no, increment and check next.
...
var whoEatsWho = function(zoo) {
    ...
    while (i < zoo.length) {
        i++
        // Check left:
        if (i !== 0 && (checkIfEats(zoo[i], zoo[i-1]))) {
            result.push(`${zoo[i]} eats ${zoo[i-1]}`)
            zoo.splice(i-1, 1);
            i = -1;
            }
        // Check right:
        if (i !== zoo.length - 1 && checkIfEats(zoo[i], zoo[i+1])) {
            result.push(`${zoo[i]} eats ${zoo[i+1]}`)
            zoo.splice(i+1, 1);
            i = -1;
        }
