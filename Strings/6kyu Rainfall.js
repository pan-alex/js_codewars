...
// P:
    // town: String representing a town in towns
    // strng:
        // String containing data for several cities.
        // Cities are separated by \n
        // Data are separated from the city by :
        // Months are separated from each other by a ,
        // Month labels are separated from data by a space ' '
// R: Mean or variance of the given town
...
// P:
    // Process data:
        // Convert data to an object
            // Each city is a property
            // Split string by \n
        // The value for the property is an array of numbers corresponding to the rainfall for Jan-Dec

    // calculate mean and variance


function dataToObject(strng) {
    let dataObj = {}
    let arr = strng.split('\n')
    for(i in arr) {
        let city = arr[i].split(':')[0]
        let monthlyData = arr[i].split(',').map( x => +x.split(' ')[1])
        dataObj[city] = monthlyData
    }
    return dataObj
}

function mean(town, strng) {
    let dataObj = dataToObject(strng)
    if ( !(town in dataObj)) return -1
    let avg = dataObj[town].reduce( (sum, x) => sum + x, 0) / 12
    return avg
}
...
function variance(town, strng) {
    ...
    let variance = dataObj[town].reduce( (sum, x) => sum + (avg - x)**2, 0) / 12
    return variance
}
